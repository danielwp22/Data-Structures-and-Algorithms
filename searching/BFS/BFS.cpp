...
public:
    ...
    vector<int> BreadthFirstSearch(){
        ...
        vector<Node*> myQueue = {parent};
        while(myQueue.size() != 0){
            Node *current_node = myQueue[0];
            myQueue.erase(myQueue.begin());
            list.push_back(current_node->value);
            cout << current_node->value << endl;
            if (current_node->left != NULL){
                myQueue.push_back(current_node->left);
                
            }
            if (current_node->right != NULL){
                myQueue.push_back(current_node->right);
            }
        }
        return list;
    }
...
int main(){
    ...
    tree.lookup(170);
    tree.lookup(22);
    vector<int> list = tree.BreadthFirstSearch();
    cout << "list: " << endl;
    for (int num : list){
